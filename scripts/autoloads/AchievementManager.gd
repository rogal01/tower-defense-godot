extends Node

signal achievement_unlocked(achievement: Dictionary)

const ACHIEVEMENTS: Array = [
	{id="first_kill", title="First Blood", desc="Kill your first enemy", emoji="🗡️"},
	{id="wave_5", title="Survivor", desc="Reach wave 5", emoji="🛡️"},
	{id="wave_10", title="Veteran", desc="Reach wave 10", emoji="⚔️"},
	{id="wave_20", title="Legend", desc="Reach wave 20", emoji="👑"},
	{id="wave_30", title="Immortal", desc="Reach wave 30", emoji="🏆"},
	{id="wave_50", title="Mythic", desc="Reach wave 50", emoji="🌟"},
	{id="kills_50", title="Slayer", desc="Kill 50 enemies", emoji="💀"},
	{id="kills_200", title="Destroyer", desc="Kill 200 enemies", emoji="🔥"},
	{id="kills_500", title="Annihilator", desc="Kill 500 enemies", emoji="💥"},
	{id="combo_10", title="Combo King", desc="Get a 10x combo", emoji="🔗"},
	{id="combo_20", title="Combo God", desc="Get a 20x combo", emoji="⛓️"},
	{id="boss_kill", title="Boss Slayer", desc="Kill your first boss", emoji="☠️"},
	{id="5_bosses", title="Boss Hunter", desc="Kill 5 bosses in one run", emoji="🐉"},
	{id="10_bosses", title="Boss Legend", desc="Kill 10 bosses in one run", emoji="🐲"},
	{id="5_towers", title="Architect", desc="Place 5 towers", emoji="🏗️"},
	{id="10_towers", title="Fortress", desc="Place 10 towers", emoji="🏰"},
	{id="all_tower_types", title="Arsenal", desc="Place all tower types", emoji="🎯"},
	{id="use_power", title="Sorcerer", desc="Use a power for the first time", emoji="✨"},
	{id="max_tower", title="Master Builder", desc="Upgrade a tower to level 5", emoji="⬆️"},
	{id="rich", title="Rich", desc="Have 500 gold at once", emoji="💰"},
	{id="rich_1000", title="Millionaire", desc="Have 1000 gold at once", emoji="🤑"},
	{id="gold_hoarder", title="Gold Hoarder", desc="Have 2000 gold at once", emoji="🏦"},
	{id="score_1000", title="Score Chaser", desc="Reach 1000 score", emoji="📊"},
	{id="diamond_10", title="Diamond Hoarder", desc="Earn 10 diamonds in a run", emoji="💎"},
	{id="diamond_50", title="Diamond Mine", desc="Earn 50 diamonds in one run", emoji="💎"},
	{id="repaired_3", title="Mechanic", desc="Repair the base 3 times in a run", emoji="🔧"},
	{id="upgrade_all", title="Well Rounded", desc="Buy all 4 player upgrades", emoji="🎖️"},
	{id="endless_10", title="Endurance", desc="Reach wave 10 in endless mode", emoji="♾️"},
	{id="kills_1000", title="Genocide", desc="Kill 1000 enemies in one run", emoji="👻"},
	{id="wave_100", title="Centurion", desc="Reach wave 100", emoji="⚔️"},
	{id="no_damage", title="Untouchable", desc="Complete a wave without base taking damage", emoji="🛡️"},
	{id="speed_demon", title="Speed Demon", desc="Beat wave 10 on 3x speed", emoji="💨"},
	{id="all_powers", title="Elementalist", desc="Use all 4 powers in one run", emoji="🌊"},
	{id="survivor_1hp", title="Last Stand", desc="Win a wave with base at 1 HP", emoji="❤️"},
	{id="campaign_5", title="Campaigner", desc="Complete 5 campaign levels", emoji="🗺️"},
	{id="campaign_10", title="Strategist", desc="Complete 10 campaign levels", emoji="🏅"},
	{id="campaign_all", title="Conqueror", desc="Complete all campaign levels", emoji="👑"},
	{id="campaign_no_damage", title="Flawless", desc="Beat a campaign level without base damage", emoji="🛡️"},
	{id="campaign_3star", title="Perfectionist", desc="Beat 5 campaign levels at full base HP", emoji="⭐"},
]

var _powers_used_this_run: Dictionary = {}
var _kills_this_run: int = 0
var _bosses_killed_this_run: int = 0
var _diamonds_this_run: int = 0

func reset_run() -> void:
	_powers_used_this_run.clear()
	_kills_this_run = 0
	_bosses_killed_this_run = 0
	_diamonds_this_run = 0

func restore_run_progress(kills: int, bosses: int, diamonds: int, powers_used: Dictionary, _ability_types: Array) -> void:
	_powers_used_this_run.clear()
	for key in powers_used.keys():
		_powers_used_this_run[int(key)] = bool(powers_used[key])
	_kills_this_run = maxi(0, kills)
	_bosses_killed_this_run = maxi(0, bosses)
	_diamonds_this_run = maxi(0, diamonds)

func check(id: String) -> void:
	if SaveManager.is_achievement_unlocked(id):
		return
	SaveManager.unlock_achievement(id)
	var ach := _find(id)
	if not ach.is_empty():
		achievement_unlocked.emit(ach)

func _find(id: String) -> Dictionary:
	for a in ACHIEVEMENTS:
		if a["id"] == id:
			return a
	return {}

func is_unlocked(id: String) -> bool:
	return SaveManager.is_achievement_unlocked(id)

func get_all() -> Array:
	return ACHIEVEMENTS

func on_enemy_killed(enemy_type: int, _tower_type: int, combo: int) -> void:
	_kills_this_run += 1
	var total := SaveManager.get_stat("lifetime_kills") + _kills_this_run
	check("first_kill")
	if _kills_this_run >= 50: check("kills_50")
	if _kills_this_run >= 200: check("kills_200")
	if _kills_this_run >= 500: check("kills_500")
	if total >= 1000: check("kills_1000")
	if combo >= 10: check("combo_10")
	if combo >= 20: check("combo_20")
	if enemy_type == GameData.EnemyType.BOSS:
		_bosses_killed_this_run += 1
		check("boss_kill")
		if _bosses_killed_this_run >= 5: check("5_bosses")
		if _bosses_killed_this_run >= 10: check("10_bosses")

func on_wave_complete(wave: int, base_hp: float, _max_base_hp: float, base_hp_before: float, is_endless: bool, _map_type: int, _is_randomizer: bool, game_speed: int) -> void:
	if wave >= 5: check("wave_5")
	if wave >= 10: check("wave_10")
	if wave >= 20: check("wave_20")
	if wave >= 30: check("wave_30")
	if wave >= 50: check("wave_50")
	if wave >= 100: check("wave_100")
	if base_hp >= base_hp_before: check("no_damage")
	if base_hp <= 1.0: check("survivor_1hp")
	if game_speed >= 3 and wave >= 10: check("speed_demon")
	if is_endless and wave >= 10: check("endless_10")

func on_tower_placed(tower_count: int, tower_types_set: Array) -> void:
	if tower_count >= 5: check("5_towers")
	if tower_count >= 10: check("10_towers")
	if tower_types_set.size() >= GameData.CANONICAL_TOWER_TYPES.size(): check("all_tower_types")

func on_power_used(power_type: int) -> void:
	check("use_power")
	_powers_used_this_run[power_type] = true
	if _powers_used_this_run.size() >= 4:
		check("all_powers")

func on_ability_used(_tower_type: int) -> void:
	pass

func on_tower_maxed() -> void:
	check("max_tower")

func on_all_towers_upgraded(towers_count: int, upgraded_count: int) -> void:
	if towers_count > 0 and towers_count == upgraded_count:
		check("upgrade_all")

func on_gold_checked(gold: int) -> void:
	if gold >= 500: check("rich")
	if gold >= 1000: check("rich_1000")
	if gold >= 2000: check("gold_hoarder")

func on_score_checked(score: int) -> void:
	if score >= 1000: check("score_1000")

func on_diamonds_earned(count: int) -> void:
	_diamonds_this_run += count
	if _diamonds_this_run >= 10: check("diamond_10")
	if _diamonds_this_run >= 50: check("diamond_50")

func on_base_healed(repairs_this_run: int) -> void:
	if repairs_this_run >= 3:
		check("repaired_3")

func on_campaign_result(_level_id: int, _stars: int, no_damage: bool, beaten_count: int, flawless_count: int, total_levels: int) -> void:
	if beaten_count >= 5: check("campaign_5")
	if beaten_count >= 10: check("campaign_10")
	if beaten_count >= total_levels: check("campaign_all")
	if no_damage: check("campaign_no_damage")
	if flawless_count >= 5: check("campaign_3star")

func on_trap_placed(_traps_this_run: int) -> void:
	pass

func on_mine_triple_kill() -> void:
	pass

func on_bounty_complete(_total_completed: int) -> void:
	pass

func on_boss_rush(_bosses_defeated: int) -> void:
	pass

func on_prestige() -> void:
	pass

func on_maps_played(_maps_count: int) -> void:
	pass

func on_wave_5_no_towers() -> void:
	pass
