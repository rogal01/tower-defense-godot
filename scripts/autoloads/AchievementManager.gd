# AchievementManager.gd — Event-Driven Achievement System
# Manages the 39 canonical achievements synchronized with the Kotlin ContentCatalog.
# Provides a decoupled interface for game events to trigger persistent unlocks.
extends Node
...
const ACHIEVEMENTS: Array = [
	...
	{id="upgrade_all",      title="Well Rounded",      desc="Buy all 4 player upgrades",              emoji="🏅"},
...
func restore_run_progress(kills: int, bosses: int, diamonds: int, powers_used: Dictionary) -> void:
	_powers_used_this_run.clear()
...
# Main unlock entry point. Uses a decoupled ID-based check to ensure
# persistence is updated only when necessary. Fires signals for UI feedback.
...
func on_enemy_killed(enemy_type: int, _tower_type: int, combo: int) -> void:
	...
	if _kills_this_run >= 1000: check("kills_1000")
...
func on_wave_complete(wave: int, base_hp: float, _max_base_hp: float, base_hp_before: float,
		is_endless: bool, _map_type: int, _is_randomizer: bool, game_speed: int) -> void:
...
func on_upgrade_purchased(_upgrade_id: String, all_count: int) -> void:
	if all_count >= 4: check("upgrade_all")
